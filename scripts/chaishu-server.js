...
async function createFeishuDoc(markdown, fileName) {
  ...
  // Write HTML to temp file for Puppeteer
  const tmpDir = path.join(VAULT, 'cards', '.tmp');
  if (!fs.existsSync(tmpDir)) fs.mkdirSync(tmpDir, { recursive: true });
  const htmlPath = path.join(tmpDir, `${fileName}.html`);
  const pngPath = path.join(tmpDir, `${fileName}.png`);
  ...
  try {
    ...
    await page.setViewport({ width: 720, height: Math.min(bodyHeight + 40, 16384) });
    await page.screenshot({ path: pngPath, type: 'png', fullPage: true });
    logger.info('HTML rendered to PNG', { path: pngPath, height: bodyHeight });
  } finally {
    await browser.close();
...
function buildReadPage(markdown, bookTitle) {
  // Strip frontmatter
  let body = markdown.replace(/^---\n[\s\S]*?\n---\n*/, '');
  const safeTitle = bookTitle || '拆书';
  ...
  for (const line of lines) {
    // Headers
    if (/^### \d️⃣/.test(line)) {
      blocks.push(`<h3 class="kp-title">${line.replace(/^### /, '')}</h3>`);
      continue;
    }
    if (/^## (.+)/.test(line)) {
      const title = line.replace(/^## /, '');
      blocks.push(`<h2 class="section-title">${title}</h2>`);
      continue;
    }
    if (/^# (.+)/.test(line)) {
      blocks.push(`<h1 class="main-title">${line.replace(/^# /, '')}</h1>`);
      continue;
    }

    // Blockquote (insights)
    if (/^> 🔑/.test(line)) {
      blocks.push(`<div class="insight-box"><span class="insight-icon">💡</span>${line.replace(/^> 🔑 \*\*关键洞见：\*\*\s*/, '')}</div>`);
      continue;
    }
    if (/^> \*\*/.test(line)) {
      blocks.push(`<div class="insight-box secondary"><span class="insight-icon">💬</span>${line.replace(/^> \*\*(.+?)\*\*\s*/, '$1')}</div>`);
      continue;
    }
    if (/^> (.+)/.test(line)) {
      blocks.push(`<blockquote>${line.replace(/^> /, '')}</blockquote>`);
      continue;
    }

    // Practice
    if (/^🫁/.test(line)) {
      blocks.push(`<div class="practice">${line.replace(/\*\*/g, '')}</div>`);
      continue;
    }
    ...
    // Checkbox
    if (/^- \[ \] (.+)/.test(line)) {
      blocks.push(`<li class="action-item">${line.replace(/^- \[ \] /, '')}</li>`);
      continue;
    }
    ...
    if (/^\| .+ \| 《.+》 \| .+ \|$/.test(line)) {
      const m = line.match(/^\| (.+?) \| 《(.+?)》 \| (.+?) \|$/);
      if (m) {
        const icon = m[1].includes('横向') ? '↔' : m[1].includes('深度') ? '↗' : '↘';
        blocks.push(`<div class="rec-item"><span class="rec-icon">${icon}</span><div><strong>《${m[2]}》</strong><br><span class="rec-meta">${m[1]} · ${m[3]}</span></div></div>`);
      }
      continue;
    }

    // Skip table headers/separators
    if (/^\|[-| ]+\|$/.test(line) || /^\| 方向 \|/.test(line)) continue;
    ...
    // Regular text
    if (line.trim()) {
      blocks.push(`<p>${line.replace(/\*\*(.+?)\*\*/g, '<strong>$1</strong>').replace(/\*(.+?)\*/g, '<em>$1</em>')}</p>`);
    }
  }
...
<style>
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
  ...
  .divider{
    border: none;
    height: 1px;
    background: linear-gradient(to right, transparent, #d5c8b8, transparent);
    margin: 24px 0;
  }
  .action-item{
    list-style: none;
  ...
  .rec-meta{ font-size: 13px; color: #8b7d6b; }
  .rec-item strong{ font-size: 15px; }
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === 'GET' && url.pathname.startsWith('/read/')) {
    const bookFile = decodeURIComponent(url.pathname.replace('/read/', ''));
    const markdown = bookCache.get(bookFile);
    if (markdown) {
      res.setHeader('Content-Type', 'text/html; charset=utf-8');
      return res.end(buildReadPage(markdown, bookFile));
    }
    res.statusCode = 404;
    return res.end('<h1>404</h1><p>书籍内容未找到，可能已过期。</p>');
  }
