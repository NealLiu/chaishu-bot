...
// ── Format as Plain Text (for .txt upload, no markdown symbols) ──
function formatPlainText(md) {
  let t = md.replace(/^---\n[\s\S]*?\n---\n*/, ''); // strip frontmatter
  const hr = '────────────────────────────────────────────';

  // Headers → decorated title
  t = t.replace(/^#### (.+)$/gm, '\n  $1');
  t = t.replace(/^### \d️⃣ (.+)$/gm, '\n\n▍$1');
  t = t.replace(/^## (.+)$/gm, `\n\n${hr}\n  $1\n${hr}`);
  t = t.replace(/^# (.+)$/gm, `\n\n${'═'.repeat(40)}\n  $1\n${'═'.repeat(40)}`);
  ...
  // Italic markers → plain
  t = t.replace(/\*(.+?)\*/g, '「$1」');

  // Blockquotes → indented
  t = t.replace(/^> 🔑 \*\*关键洞见：\*\*\s*/gm, '\n  💡 关键洞见：');
  t = t.replace(/^> \*\*(.+?)\*\*\s*/gm, '\n  💬 $1');
  t = t.replace(/^> (.+)$/gm, '  $1');
